...
import LocationService from '../services/LocationService'; // Importation pour récupérer les événements
...
const userRole = ref('');
const userLogin = ref('');
const activeMenuKey = ref('clients'); // Activer l'item "clients" dans le menu
...
// Configuration des colonnes du tableau (AVEC ADRESSE)
const tableColumns = [
  {
    title: 'Nom Complet',
    key: 'nomComplet',
...
const filteredClients = computed(() => {
  ...
  return clients.value.filter(client => 
    ...
    client.emailCli?.toLowerCase().includes(query) ||
    client.telephoneCli?.includes(query) ||
    client.addresseCli?.toLowerCase().includes(query) // Ajout de la recherche par adresse
  );
});
...
onMounted(() => {
  ...
  fetchClientUsers();
  fetchMetrics();
  fetchAllEvents(); // Charger tous les événements pour calculer les clients actifs
});
...
async function saveClient() {
  try {
    ...
    await fetchClients();
    await fetchMetrics();
    await fetchAllEvents(); // Recharger les événements pour mettre à jour les stats
  } catch (error) {
    console.error("Erreur de sauvegarde:", error);
  } finally {
    isSubmitting.value = false;
...
const refreshPage = async () => {
  await fetchClients();
  await fetchMetrics();
  await fetchAllEvents(); // Recharger aussi les événements
};
