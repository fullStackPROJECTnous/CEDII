...
const fetchClientData = async () => {
    ...
    clientName.value = 'Randria Malaso'; 
    clientInfo.value = {
        name: 'Randria Malaso',
        email: 'malaso@example.com',
        phone: '+261 02 563 23',
        address: 'Antarandolo Fianarantsoa',
    ...
    pastReservations.value = [
        { id: 98, resource: 'Projecteur', startDate: '2025-09-01', endDate: '2025-09-05', status: 'Terminée' },
        { id: 85, resource: 'Salle de réunion', startDate: '2025-07-20', endDate: '2025-07-20', status: 'Terminée' },
    ];
};
...
<template>
  <div class="vh-100 d-flex flex-column"> 
    ...
    <main class="main-content flex-grow-1 overflow-auto bg-light p-4"> 
      ...
      <div v-if="!clientInfo.name && authStore.isAuthenticated" class="text-center py-5">
        <div class="spinner-border text-primary" role="status">
            <span class="visually-hidden">Chargement...</span>
        </div>
        <p class="mt-3 text-muted">Chargement de votre compte...</p>
      </div>
      
      <div v-if="!clientInfo.name && authStore.isAuthenticated">
          <h2 class="text-danger">DEBUG: Données Store (Avant Profil)</h2>
          <pre>{{ JSON.stringify(authStore.clientData, null, 2) }}</pre>
      </div>
      <div v-else-if="clientInfo.name">
          <header class="pb-3 border-bottom d-flex justify-content-between align-items-center">
            <h1 class="text-secondary">Mon Compte Client 👤</h1>
            <p class="text-muted mb-0">Bienvenue, {{ clientName }} !</p>
          </header>
      ...
      <div v-else-if="!authStore.isAuthenticated" class="text-center py-5">
        <p class="text-muted">Vous n'êtes pas connecté. Redirection...</p>
      </div>
    </main>
  </div>
</template>
...
// Les infos de l'utilisateur proviendront de l'objet clientInfo
const clientName = ref('Client'); 
const clientInfo = ref({}); 
const nextReservation = ref({});
const pastReservations = ref([]);

const loadClientDataFromStore = async () => { 
    // userData contient l'objet de connexion : { idUti: 9, loginUti: "Mimi", ... }
    const userData = authStore.clientData; 

    if (userData && userData.idUti) { 
        
        try {
            // 🚨 CORRECTION CRITIQUE : Utilisation de getMyProfile()
            // Cette fonction appelle la route générique '/profile'.
            // Le back-end utilise l'ID du Token JWT pour trouver le client, 
            // ce qui résout les problèmes de cache d'ID et d'erreurs de récupération d'ID.
            const clientProfile = await ClientService.getMyProfile(); 
            // Note : getMyProfile() est censé retourner directement l'objet de données (pas besoin de .data ou .data || ...)

            // 2. UTILISE LES CLÉS EXACTES DE VOTRE BDD (nomCli, prenomCli, emailCli, etc.)
            clientName.value = `${clientProfile.prenomCli || ''} ${clientProfile.nomCli || ''}`.trim() || userData.loginUti;

            clientInfo.value = {
                name: clientName.value, 
                email: clientProfile.emailCli || 'N/A', 
                phone: clientProfile.telephoneCli || 'N/A',
                address: clientProfile.adresseCli || 'N/A',
                
                 
            };
            
            // 3. Charger les réservations une fois les infos client obtenues
            // IMPORTANT : Utiliser l'idCli qui vient d'être récupéré via l'API.
            fetchReservations(clientProfile.idCli); 

        } catch (error) {
            // 🚨 CONSOLE LOG POUR LE DEBUG : Regardez l'objet 'error' pour le code HTTP (404, 500, etc.)
            console.error("Échec de la récupération du profil client via getMyProfile. Détails de l'erreur:", error.response?.data || error);
            
            // Revenir aux données minimales de la connexion en cas d'échec
            clientName.value = userData.loginUti;
            clientInfo.value = { 
                name: userData.loginUti, 
                email: 'Erreur: Profil non chargé', 
                phone: 'N/A', 
                address: 'N/A', 
...
const fetchReservations = async (clientId) => {
    console.log("Fetching reservations for client ID:", clientId);

    // 🚨 REMPLACEZ LA SIMULATION PAR UN VÉRITABLE APPEL API AU BACK-END
    // Qui charge les réservations de l'utilisateur (utilisez getClientHistory du ClientService)
    
    // Simulation actuelle (à remplacer par API)
    nextReservation.value = {
        id: 101,
...
const formatDate = (dateString) => {
    ...
    // Gère le format ISO (ex: 2024-01-15)
...
onMounted(() => {
    ...
    // 🚨 Appel asynchrone qui charge toutes les données
    loadClientDataFromStore(); 
});

/*watchEffect(() => {
    if (authStore.isAuthenticated && authStore.clientData) {
        loadClientDataFromStore();
    }
});*/
</script>
