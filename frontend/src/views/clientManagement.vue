...
<template>
  <div class="dashboard-wrapper"> 
            ...
            <n-card class="filters-card custom-card mb-4">
              <div class="d-flex justify-content-between align-items-center">
                <div class="d-flex align-items-center">
                  ...
                  <!-- NOUVEAU BOUTON: NOUVEL UTILISATEUR -->
                  <n-button 
                    type="success" 
                    @click="openUserModal('add')"
                    class="me-3 custom-btn-success"
                  >
                    <template #icon>
                      <n-icon><i class="bi bi-person-plus"></i></n-icon>
                    </template>
                    Nouvel Utilisateur
                  </n-button>
                </div>
    ...
    <!-- MODAL SIMPLIFIÉE: CRÉATION D'UTILISATEUR CLIENT -->
    <n-modal
      v-model:show="isUserModalVisible"
      preset="dialog"
      :title="'Créer un Nouvel Utilisateur Client'"
      positive-text="Créer"
      negative-text="Annuler"
      :loading="userSubmitting"
      @positive-click="saveUser"
      @negative-click="isUserModalVisible = false"
      style="width: 500px"
      class="scrollable-modal"
    >
      <div class="modal-scroll-content">
        <n-form :model="currentUser" :rules="userRules" ref="userFormRef">
          <n-grid :cols="1" :x-gap="16" :y-gap="16">
            <!-- Login -->
            <n-gi>
              <n-form-item label="Login" path="loginUti" required>
                <n-input 
                  v-model:value="currentUser.loginUti" 
            ...
            <!-- Mot de passe -->
            <n-gi>
              <n-form-item label="Mot de passe" path="passwordUti" required>
                <n-input 
                  v-model:value="currentUser.passwordUti" 
                  type="password"
            ...
            <!-- Rôle (fixé à CLIENT) -->
            <n-gi>
              <n-form-item label="Rôle">
                <n-input
                  :value="'CLIENT'"
                  disabled
                  placeholder="Rôle fixé à CLIENT"
                />
              </n-form-item>
            </n-gi>
            ...
            <!-- Information -->
            <n-gi>
              <n-alert type="info" title="Information" class="mb-0">
                <div class="small">
                  <i class="bi bi-lightbulb me-1"></i>
                  Après la création, vous pourrez associer cet utilisateur à un client dans le formulaire "Ajouter un Client".
                </div>
              </n-alert>
            </n-gi>
          </n-grid>
...
// Modèle de données SIMPLIFIÉ pour utilisateur client
const emptyUser = {
  loginUti: '',
  passwordUti: '',
  emailUti: '',
  roleUti: 'CLIENT', // Fixé à CLIENT
  statutUti: 'actif'
};
...
const clientUserOptions = computed(() => {
  return clientUsers.value.map(user => {
    // REMPLACEZ 'login' par le nom exact de la propriété que vous voyez dans la console
    const login = user.login || user.loginUti || user.username || user.email || `User${user.idUti}`;
    
    return {
      label: `ID ${user.idUti} - ${login}`,
      value: user.idUti
    };
  });
});

// Règles de validation pour client
const clientRules = {
  nomCli: {
    required: true,
  ...
  telephoneCli: {
    ...
    validator: (rule, value) => {
      if (!value || value.trim().length === 0) {
        return new Error('Le téléphone est requis');
      }
      // Validation basique pour numéro de téléphone
      if (!/^[0-9+\s()-]+$/.test(value)) {
        return new Error('Format de téléphone invalide');
      }
      return true;
    }
  },
...
// RÈGLES DE VALIDATION SIMPLIFIÉES POUR UTILISATEUR CLIENT
const userRules = {
  loginUti: {
    required: true,
...
async function fetchClientUsers() {
  try {
    ...
    console.log('Utilisateurs clients chargés:', clientUsers.value.length);
  } catch (error) {
...
// FONCTION CORRIGÉE: Ouvrir la modal client
function openModal(mode, clientData = null) {
  ...
  if (clientFormRef.value) {
    clientFormRef.value.restoreValidation();
  }
}

// NOUVELLE FONCTION: Ouvrir la modal utilisateur SIMPLIFIÉE
...
// FONCTION CORRIGÉE: Sauvegarder un client
async function saveClient() {
  try {
    ...
    const clientData = {
      ...
      idUti: currentClient.value.idUti || null,
      statutCli: currentClient.value.statutCli || 'actif',
      typeCli: currentClient.value.typeCli || 'particulier'
    };
    ...
    if (modalMode.value === 'create') {
      ...
      if (!clientData.idCli) {
        throw new Error('ID client manquant pour la modification');
      }
      const response = await ClientService.updateClient(clientData.idCli, clientData);
    ...
    await fetchClients();
    await fetchMetrics();
    await fetchClientUsers(); // Recharger aussi les utilisateurs
  } catch (error) {
...
// NOUVELLE FONCTION: Sauvegarder un utilisateur CLIENT
async function saveUser() {
  try {
    userSubmitting.value = true;
    
    // Préparer les données MINIMALES pour l'utilisateur CLIENT
    const userData = {
      loginUti: currentUser.value.loginUti,
      motDePasseUti: currentUser.value.passwordUti,
      roleUti: 'client' // ⚠️ IMPORTANT: en minuscules
      // Pas besoin d'email, nom, prénom, etc.
    };
    
    console.log("Données envoyées:", userData);
    
    // Valider les champs obligatoires
    if (!userData.loginUti || !userData.motDePasseUti) {
      toast.error("Le login et le mot de passe sont obligatoires");
      userSubmitting.value = false;
      return;
    }
    
    // Vérifier la longueur minimale du mot de passe
    if (userData.motDePasseUti.length < 6) {
      toast.error("Le mot de passe doit contenir au moins 6 caractères");
      userSubmitting.value = false;
      return;
    }
    
    // Utiliser la fonction saveUser() du service
    const response = await UserServices.saveUser(userData);
    
    console.log("Réponse API:", response);
    
    toast.success(`Utilisateur "${userData.loginUti}" créé avec succès !`, {
      timeout: 3000
    });
    ...
    // Recharger la liste des utilisateurs pour les options de sélection
    await fetchClientUsers();
    
    // Afficher une info pour indiquer comment associer à un client
    toast.info(`Utilisateur créé. Vous pouvez maintenant lui associer un client.`, {
      timeout: 5000
    });
...
/* NOUVEAU STYLE BOUTON SUCCÈS POUR NOUVEL UTILISATEUR */
.custom-btn-success {
