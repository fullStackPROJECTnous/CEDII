  ...
  <div class="dashboard-wrapper">
    <n-layout has-sider class="h-100">
      ...
      <n-layout class="main-content">
        <!-- Header fixe -->
        <n-layout-header bordered class="custom-header fixed-header d-flex justify-content-between align-items-center p-4">
          <div>
            <n-button 
              @click="goBack" 
              type="default"
        ...
        <!-- Contenu avec scroll vertical -->
        <n-layout-content class="content-with-header p-4 bg-light">
...
const calculateSuccessRate = () => {
  if (rapportData.value.totalRevenue && rapportData.value.pendingAmount) {
    const total = rapportData.value.totalRevenue + rapportData.value.pendingAmount;
    return Math.round((rapportData.value.totalRevenue / total) * 100);
  }
  
  return 98; // Garder 98% comme fallback
};
...
.content-with-header {
  ...
  overflow-x: hidden !important; /* Désactiver le défilement horizontal */
  height: calc(100vh - 80px) !important; /* Hauteur moins le header */
  max-height: calc(100vh - 80px) !important;
}

/* Container pour empêcher le débordement horizontal */
.container-fluid {
  max-width: 100%;
  overflow-x: hidden;
  padding-right: 15px;
...
/* Assurer que les lignes ne créent pas de défilement horizontal */
.row {
  margin-right: -15px;
  margin-left: -15px;
  flex-wrap: nowrap; /* Change ceci en 'wrap' pour empêcher le débordement */
  flex-wrap: wrap; /* S'assurer que les éléments s'enveloppent */
}

/* Ajuster les colonnes pour éviter le débordement */
.col-md-4, .col-lg-6, .col-12, .col-md-3 {
  padding-right: 15px;
  padding-left: 15px;
...
@media (max-width: 768px) {
  ...
  /* Sur mobile, s'assurer que tout s'ajuste */
  .row {
    margin-right: -10px;
    margin-left: -10px;
  }
...
.main-content {
  overflow: hidden; /* Désactiver tout défilement sur le conteneur principal */
}
...
.content-with-header::-webkit-scrollbar {
  width: 8px;
}

.content-with-header::-webkit-scrollbar-track {
  background: #f1f1f1;
  border-radius: 4px;
}

.content-with-header::-webkit-scrollbar-thumb {
  background: #c1c1c1;
  border-radius: 4px;
}

.content-with-header::-webkit-scrollbar-thumb:hover {
  background: #a8a8a8;
}
...
/* S'assurer que les tableaux et listes ne créent pas de défilement */
:deep(.n-list),
:deep(.n-card) {
  max-width: 100%;
  overflow: hidden;
